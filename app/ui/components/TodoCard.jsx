...
// Make a motion-enabled Card component
...
export default function TodoCard({ todo, onToggleComplete, onEdit, onDelete }) {
  ...
  useEffect(() => {
    if (todo?.createdAt) {
      setDateString(new Date(todo.createdAt).toLocaleString());
    } else {
      setDateString("");
    }
  }, [todo?.createdAt]);
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Typography
            ...
            sx={{
              fontWeight: 600,
              fontSize: "1rem",
              color: todo.completed ? "#9CA3AF" : "white",
        ...
        <Stack direction="row" spacing={0.5} alignItems="center">
          <Tooltip title={todo.completed ? "Mark as incomplete" : "Mark complete"}>
            ...
            >
              <CheckCircleIcon
                sx={{
                  color: todo.completed ? "#2563EB" : "#9CA3AF",
                  fontSize: 20,
                }}
              />
            </IconButton>
